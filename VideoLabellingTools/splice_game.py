...
Example:
  python splice_game.py \
    ...
    --csv /data/labels/densified_scores/game_1_scores.csv \
    --switch-frame 25400 \
    --out-video-prefix /data/spliced_videos/game_1 \
    --out-csv-prefix /data/spliced_scores/game_1
...
def split_csv(csv_path: Path, switch_frame: int, out_prefix: Path):
    """
    Split CSV into two parts at switch_frame.
    Frame indices are preserved (no reindexing).
    """
...
def main():
    ...
    ap.add_argument("--out-video-prefix", type=Path, required=True, help="Output prefix for video files")
    ap.add_argument("--out-csv-prefix", type=Path, required=True, help="Output prefix for CSV files")
    args = ap.parse_args()
    ...
    out_vids = run_ffmpeg_cut(args.video, fps, args.switch_frame, args.out_video_prefix)
    out_csvs = split_csv(args.csv, args.switch_frame, args.out_csv_prefix)

    print(f"✅ Written video files: {out_vids[0]}, {out_vids[1]}")
    print(f"✅ Written CSV files:   {out_csvs[0]}, {out_csvs[1]}")
